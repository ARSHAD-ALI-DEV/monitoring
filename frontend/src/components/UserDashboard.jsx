...
const UserDashboard = () => {
  const { myShots, myActivity } = useShotStore();
  const [pressedShot, setPressedShot] = useState(null);
  let pressTimer = null; // Press टाइमर ट्रैक करने के लिए

  useEffect(() => {
    myActivity(); // पहली बार API कॉल

    const interval = setInterval(() => {
      myActivity(); // हर 30 सेकंड में API से डेटा लाओ
    }, 30000);

    return () => clearInterval(interval); // क्लीनअप
  }, []);
  ...
  return (
    <div className="bg-white min-h-screen p-5">
      <h2 className="text-3xl font-bold text-indigo-800 mb-5">My Screenshots</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
        {myShots.length > 0 ? (
          myShots.map((shot, index) => (
            <div
              key={index}
